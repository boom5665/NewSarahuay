...
const i18n = require('nuxtseo-shared/i18n');
...
function setupDevToolsUI(_options, resolve, nuxt) {
  devtools.setupDevToolsUI({ route: "/__nuxt-robots", name: "nuxt-robots", title: "Robots", icon: "carbon:bot" }, resolve, nuxt);
}
...
function mapPathForI18nPages(path, autoI18n) {
  ...
  function resolveForAllLocales(pageName, pageLocales) {
    return autoI18n.locales.filter((l) => {
      ...
      return ufo.withLeadingSlash(i18n.generatePathForI18nPages({
        localeCode: l.code,
...
const module$1 = kit.defineNuxtModule({
  ...
  async setup(config, nuxt) {
    ...
    const resolvedAutoI18n = typeof config.autoI18n === "boolean" ? false : config.autoI18n || await i18n.resolveI18nConfig();
...
`);
    ...
    const nitroPreset = kit$2.resolveNitroPreset(nuxt.options.nitro);
    const contentVersion = await kit$2.resolveNuxtContentVersion();
    ...
    nuxt.hook("modules:done", async () => {
      ...
      if (config.header) {
        ...
        for (const path of noIndexPaths) {
          kit.extendRouteRules(path, { robots: "noindex" }, { override: true });
        }
      ...
      if (resolvedAutoI18n && resolvedAutoI18n.locales && resolvedAutoI18n.strategy !== "no_prefix") {
        const i18n$1 = resolvedAutoI18n;
        for (const group of config.groups.filter((g) => !g._skipI18n)) {
          group.allow = util.asArray(group.allow || []).map((path) => {
            if (typeof path !== "string")
              return path;
            return mapPathForI18nPages(path, i18n$1) || i18n.splitPathForI18nLocales(path, i18n$1);
          }).flat();
    ...
    if ((kit$2.isNuxtGenerate() || isFirebase) && config.robotsTxt) {
      kit.addPrerenderRoutes("/robots.txt");
      if (isFirebase)
